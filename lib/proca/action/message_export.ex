defmodule Proca.Action.MessageExport do
  ...
  def export(target_id, opts \\ []) do
    include_duplicates = Keyword.get(opts, :include_duplicates, false)
    rows = fetch_messages(target_id, include_duplicates)

    if rows == [] do
      {:error, "No messages found for target #{target_id}"}
    else
      {:ok, build_csv(rows)}
    end
  end

  defp fetch_messages(target_id, include_duplicates) do
    base =
      from(m in Message,
