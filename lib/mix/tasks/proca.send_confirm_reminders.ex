defmodule Mix.Tasks.Proca.SendConfirmReminders do
  ...
  Per-org reminder config (org.config["reminder"]):
    ...
    - "delay_days" - days between reminders (default 2)
    - "max_count"  - maximum reminders per action (default 3)
    ...
    if due == [] do
      ...
      Enum.each(due, fn {org, actions} ->
        ...
        Enum.each(actions, fn action ->
          Mix.shell().info(
            "    action #{action.id} — #{action.supporter.email} (inserted_at=#{action.inserted_at})"
          )
        end)
      end)
    end
