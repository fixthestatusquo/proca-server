defmodule Proca.Server.KeysTest do
  ...
  test "Server returns :ignore when given non-existing org" do
    p = GenServer.start_link(Proca.Server.Keys, "foobar")
    assert p = :ignore
  end

  test "When instance org has no keys it starts but will exit", %{red_org: org} do
  ...
  test "Server works when instance org has keys", %{red_org: org} do
    ...
    PublicKey.activate_for(org, k.id) |> Repo.transaction()
  ...
  test "I can get keys for encryption from server", %{red_org: o1, yellow_org: o2} do
    ...
    PublicKey.activate_for(o1, pk1.id) |> Repo.transaction()
    PublicKey.activate_for(o2, pk2.id) |> Repo.transaction()
