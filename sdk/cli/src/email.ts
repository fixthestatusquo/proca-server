...
function syncAction(action, argv, config) {
    return __awaiter(this, void 0, void 0, function () {
        var conn, tmplId, vars, res;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    console.log('bb');
                    conn = connect();
                    tmplId = process.env['MAILJET_TEMPLATE'];
                    console.log(tmplId);
                    if (!tmplId)
                        throw new Error('No MAILJET_TEMPLATE set');
                    vars = varsFromAction(action);
                    return [4 /*yield*/, conn.post("send").request({
                            FromEmail: 'dump@cahoots.pl',
                            FromName: 'Test',
...
function connect() {
    var mjKey = process.env['MAILJET_KEY'];
    var mjSec = process.env['MAILJET_SECRET'];
    if (!mjKey || !mjSec) {
        throw new Error('No MAILJET_KEY or MAILJET_SECRET set');
    }
    var conn = node_mailjet_1["default"].connect(mjKey, mjSec);
    return conn;
}
...
function varsFromAction(action) {
    var vars = {
        ...
        ref: action['contact']['ref'],
        campaign_name: action['campaign']['name'],
        campaign_title: action['campaign']['title'],
        action_page_name: action['actionPage']['name'],
