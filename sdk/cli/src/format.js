...
class Terminal {
    ...
    error(err) {
        if (err.response && err.response.errors) {
            const x = err.response.errors.map((e) => {
...
class Csv extends Terminal {
    ...
    action(a) {
        ...
        input = [[
                ...
                a.actionId,
                a.privacy.optIn
            ].concat(this.fields.map((f) => this.getField(a, f)))];
        return sync_1.default(input, opts).slice(0, -1); // chomp newline
    }
}
function getFormatter(argv) {
    if (argv.json) {
