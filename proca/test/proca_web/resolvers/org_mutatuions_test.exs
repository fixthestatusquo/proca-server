defmodule ProcaWeb.OrgMutationsTest do
  ...
  test "Can add an org", %{user: user, org_params: p} do
    r = ProcaWeb.Resolvers.Org.add_org(0, %{input: p}, %{context: %{auth: %Proca.Auth{user: user}}})
  ...
  test "Can't add an org with incorrect params", %{user: user, org_params: p} do
    ...
    er = ProcaWeb.Resolvers.Org.add_org(0, %{input: bad_name}, %{context: %{auth: %Proca.Auth{user: user}}})
    assert {:error, %Ecto.Changeset{errors: [{:name, {"has invalid format", [validation: :format]}} | _ ]}} = er

    no_title = Map.delete(p, :title)

    er = ProcaWeb.Resolvers.Org.add_org(0, %{input: no_title}, %{context: %{auth: %Proca.Auth{user: user}}})
